...
void get_d_x(float** temp_matrix,float** matrix, float * matrix_const, int *num_vars, int *index){
    for(int i =0;i<*num_vars;i++){
        temp_matrix[i][*index] = matrix_const[i];
        for(int s=0; s<*num_vars-1;s++){
            temp_matrix[i][s+*num_vars] = temp_matrix[i][s];
        }
    }
}
...
// extracts the number between the FIRST and SECOND space (used to get index1 from "subtract 1 3")
int parse_command_first(string *command, char stop){
    int temp_int = 0;
    string temp_str = "";
    bool collecting = false; // flag: start collecting after first space
    for(int i=0;i<command->length();i++){
        if(char((*command)[i]) == stop && !collecting){
            collecting = true; // first space found, start collecting next chars
            continue;
        }
        if(collecting){
            if((*command)[i] == stop) break; // second space found, stop
            temp_str += (*command)[i];
        }
    }
...
int parse_command(string *command,char stop){
    ...
    for(int i=0;i<command->length();i++){
        if(char((*command)[i]) == stop){
            ...
            temp_str = ""; // reset so we only keep what follows the LAST occurrence of stop
...
void subtract(float** matrix,float* matrix_const, int* num_vars, int index1, int index2){
    ...
    int result =0;
    for(int k =0;k<*num_vars;k++){
        result =  matrix[index1][k] - matrix[index2][k];  // subtract coefficients of each variable
        if (result > 0 && k >0){
            cout << "+";
        }
        cout << result << "x" << k+1;
...
void add(float** matrix,float* matrix_const, int* num_vars, int index1, int index2){
    ...
    int result =0;
    for(int k =0;k<*num_vars;k++){
        result =  matrix[index1][k] + matrix[index2][k];  // add coefficients of each variable
        if (result > 0 && k >0){
            cout << "+";
        }
        cout << result << "x" << k+1;
    }
    cout << "=" << matrix_const[index1] + matrix_const[index2] << endl;  // add right-hand side constants
}
...
int main(){
    ...
    // extend_matrix(matrix, temp_matrix, &num_vars);
